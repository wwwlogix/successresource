...
function my_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '3.10', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'external_url' 			=> '', // If set, overrides default API URL and points to an external URL
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			if ( $this->plugins ) {
				...
				//add_action( 'pre_set_site_transient_update_plugins', array( $this, 'us_plugins_update' ) );
		...
		public function us_plugins_update($updates) {
					...
					foreach ( $keys as $key ) {
						if ( preg_match( '|^'.$plugin['slug'].'/|', $key ) ) {
							if ( is_plugin_active( $key ) ) {
								if(version_compare($plugins[$key]['Version'], $plugin['version'], '<')) {
									$update = new stdClass();

//									$update->external = true;
//									$update->sections = array('Description' => '', 'Changelog' => '');
//									$update->homepage = '';
//									$update->author = '';
//									$update->author_profile = '';

									$update->id = 0;
									$update->plugin = $plugin_id;
									$update->slug = $plugin['slug'];
									$update->new_version = $plugin['version'];
									$update->upgrade_notice = '';
									$update->url = $plugin['info_url'];
									$update->package = $plugin['source'];
									$update->name = $plugin['name'];

									$updates->checked[$plugin_id] = $plugin['Version'];
									$updates->response[$plugin_id] = $update;
								}
							}
						}
					}
